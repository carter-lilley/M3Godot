...
func _get_centered_glyph_pos(font: Font, ch: String, glyph_size: int) -> Vector2:
	...
	if not rids.is_empty():
		...
		if glyph_idx != 0:
			...
			return size / 2.0 - ink_size / 2.0 - ink_offset

	# Fallback: center by line metrics
	var text_size := font.get_string_size(ch, HORIZONTAL_ALIGNMENT_LEFT, -1, glyph_size)
	var baseline_y := size.y / 2.0 + (font.get_ascent(glyph_size) - font.get_descent(glyph_size)) / 2.0
	return Vector2((size.x - text_size.x) / 2.0, baseline_y)
